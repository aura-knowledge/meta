...
EXEMPT_PATTERNS = [
    # Vendored-surface delta (stibdedlom/infra#529): patterns below also match
    # the bare directory paths, because push-time diff reports symlink
    # deletions as e.g. ".stibdedlom/bin" (no trailing slash) while commit-time
    # staged listing reports their contents. The central template only matches
    # the slash-suffixed forms plus none for invocation-policy.yaml; remove
    # this delta when the central surface covers both forms.
...
# Manifest schema versions accepted by the client hooks. New manifests are @2;
# @1 is grandfathered so client repos that have not re-onboarded keep validating.
...
def _find_branch_issue(branch: str | None) -> int | None:
    """Infer the routed issue number from a branch name (first ``issue-<n>`` match)."""
    if not branch:
        return None
...
def validate_pre_commit(repo_root: Path) -> dict[str, Any]:
    """Path-only staged validation for the pre-commit hook.

    Git does not reliably expose the commit message to pre-commit hooks, so
    trailer validation lives in the commit-msg hook (``validate_commit`` with
    ``--commit-msg-file``). This checks everything else: protected branch,
    trust anchor, and that staged non-exempt paths stay inside the allowed
    paths of the routing attestation inferred from the branch name.

    Signature handling mirrors ``validate_commit`` exactly: verification runs
    only when the attestation carries a ``signature`` field, with HMAC keys
    resolved via ``load_routing_attestation_key``.
    """
    repo_root = _real(repo_root)
    ...
    changed = list_changed_files(repo_root, staged=True)
    non_exempt = [path for path in changed if not is_exempt(path)]
    if not non_exempt:
        return {
            "ok": True,
    ...
    issue = _find_branch_issue(branch)
    if issue is None:
        if branch is None:
            reason = "detached HEAD: cannot infer routed issue without a branch name"
        else:
            reason = (
                "branch name must include an issue number (e.g. feature/issue-34-*) "
                "to commit non-exempt files; the commit-msg hook binds trailers separately"
            )
        return {
            "ok": False,
            "reason": reason,
            "reason_codes": ["sdl.hook.missing_issue_in_branch"],
            "changed_paths": non_exempt,
            "branch": branch,
        }
    ...
    if not attestation:
        return {
            "ok": False,
            "reason": f"routing attestation not found for issue #{issue} (inferred from branch '{branch}')",
            "reason_codes": ["sdl.hook.missing_routing_attestation"],
            "changed_paths": non_exempt,
            "branch": branch,
        }
    ...
    blocked = [
        path
        for path in non_exempt
        if not is_under_allowed_path(path, attestation.get("allowed_paths", []), repo_root)
    ]
    if blocked:
        return {
            "ok": False,
            "reason": f"staged files outside allowed_paths for issue #{issue}: {blocked}",
            "reason_codes": ["sdl.hook.allowed_paths_violation"],
            "changed_paths": non_exempt,
            "blocked_paths": blocked,
            "branch": branch,
    ...
    return {
        "ok": True,
        "reason": f"pre-commit validated against issue #{issue} attestation",
        "reason_codes": ["sdl.hook.pre_commit_validated", *reason_codes],
        "changed_paths": non_exempt,
        "branch": branch,
        "issue": issue,
    }
...
# Upstream candidates for resolving the commit range of a new-branch push.
# origin/* entries are also the ONLY refs that confer legacy-ancestry
# exemption; local main/master are used for range selection only (local refs
# are attacker-malleable, but the local hook is --no-verify bypassable anyway
# and CI — which fetches origin — is the authoritative gate).
_ZERO_BASE_UPSTREAM_CANDIDATES = ("origin/main", "origin/master", "main", "master")
_LEGACY_EXEMPTION_REFS = ("origin/main", "origin/master")


def _resolve_zero_base_upstream(repo_root: Path, head: str) -> str | None:
    """Return a merge-base SHA between a candidate upstream and ``head``.

    Tries each candidate in order and returns the first merge-base found.
    Returns None when no candidate exists or shares history with ``head``
    (e.g. orphan repositories), in which case callers fall back to a full
    rev-list of ``head``.
    """
    for candidate in _ZERO_BASE_UPSTREAM_CANDIDATES:
        result = _run_git(repo_root, "merge-base", candidate, head)
        if result.returncode == 0 and result.stdout.strip():
            return result.stdout.strip()
    return None


def _is_ancestor_of_protected_remote(repo_root: Path, sha: str) -> bool:
    """Return True only when ``sha`` is already on a protected remote ref.

    Any non-zero exit from ``merge-base --is-ancestor`` (1 = not an ancestor,
    128 = missing ref/object) means NO exemption — fail closed. This is the
    legacy-commit exemption boundary: a commit reaches origin/main only via
    the server-side gate or an admin force-push, so ancestry (unlike author
    timestamps or working-tree manifest values) cannot be forged client-side.
    """
    for ref in _LEGACY_EXEMPTION_REFS:
        result = _run_git(repo_root, "merge-base", "--is-ancestor", sha, ref)
...
def validate_push(
    ...
    *,
    ...
    allow_legacy_exemption: bool = True,
) -> dict[str, Any]:
    """Validate all commits in a push range.

    ``allow_legacy_exemption=False`` disables the ancestry-based legacy
    exemption; the default-branch CI audit step uses this so trailer-less
    commits that reached main without passing a PR gate are still reported
    (they are ancestors of origin/main by the time the audit runs).
    """
    repo_root = _real(repo_root)
    ...
    if base == "0000000000000000000000000000000000000000":
        # New-branch push: validate only the commits the branch introduces,
        # not the entire history reachable from head (issues #512/#506).
        upstream = _resolve_zero_base_upstream(repo_root, head)
        rng = f"{upstream}..{head}" if upstream else head
        result = _run_git(repo_root, "rev-list", rng)
    else:
        result = _run_git(repo_root, "rev-list", f"{base}..{head}")
    if result.returncode != 0:
        return {
            "ok": False,
            "reason": f"git rev-list failed: {result.stderr.strip() or 'unknown error'}",
            "reason_codes": ["sdl.hook.git_error"],
        }
    ...
    for sha in rev_list:
        ...
        if not attestation_match:
            # Legacy exemption: a trailer-less commit already merged to a
            # protected remote ref (origin/main|origin/master) is pre-SDL
            # history, not a new violation. Ancestry — never author
            # timestamps or manifest values — is the exemption boundary, so
            # it cannot be forged without a protected-branch force-push.
            if allow_legacy_exemption and _is_ancestor_of_protected_remote(repo_root, sha):
                skipped_legacy.append(sha)
                continue
            failures.append(
                {
                    "sha": sha,
    ...
    if failures:
        payload: dict[str, Any] = {
            ...
            "failures": failures,
            "remediation": (
                "Only commits new to this branch are validated. Amend or rebase "
                "the listed commits to add the required SDL trailers."
            ),
        }
        if skipped_legacy:
            payload["skipped_legacy"] = skipped_legacy
        return payload
    ...
    payload = {
        ...
        "reason_codes": ["sdl.hook.validated"],
        "commit_count": len(rev_list) - len(skipped_legacy),
    }
    if skipped_legacy:
        payload["skipped_legacy"] = skipped_legacy
    return payload
...
def verify_pubkey_anchor_consistency(repo_root: Path) -> tuple[bool, str]:
    ...
    ``SDL_MEMORY_ROOT`` environment variable — never from the committed
    ...
    """
    repo_root = _real(repo_root)
    raw = os.environ.get("SDL_MEMORY_ROOT", "").strip()
    if not raw:
...
def pre_commit_cli(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Path-only SDL pre-commit validation (trailers are checked by the commit-msg hook)."
    )
    parser.add_argument("--repo-root", type=Path, default=Path.cwd())
    parser.add_argument("--output", type=Path, default=None)
    args = parser.parse_args(argv)

    result = validate_pre_commit(args.repo_root.resolve())
    _json_out(result, args.output)
    return 0 if result["ok"] else 1
...
def push_cli(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--no-legacy-exemption",
        action="store_true",
        help="disable the ancestry-based legacy exemption (default-branch audit)",
    )
    parser.add_argument("--output", type=Path, default=None)
    args = parser.parse_args(argv)
    ...
    result = validate_push(
        ...
        allow_legacy_exemption=not args.no_legacy_exemption,
    )
    _json_out(result, args.output)
    return 0 if result["ok"] else 1
